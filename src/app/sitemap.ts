...
import PAGE_DATES from "@/data/page-dates.json";
...
/**
 * When a page last changed, from src/data/page-dates.json (generated by
 * `npm run page-dates` from git history). A page missing from the file gets
 * no lastmod at all: Google treats a lastmod that is always "today" as
 * noise and ignores it, which is what this used to do on every deploy.
 */
function lastModified(path: string): Date | undefined {
  const iso = (PAGE_DATES as Record<string, string>)[path];
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticPages: MetadataRoute.Sitemap = (
    ...
    ...page,
    lastModified: lastModified(page.url.slice(SITE_URL.length) || "/"),
  }));
  ...
  const cityPages: MetadataRoute.Sitemap = CITY_PAGES.map((city) => ({
    ...
    lastModified: lastModified(`/web-design/${city.slug}`),
  ...
  const servicePages: MetadataRoute.Sitemap = SERVICES.map((service) => ({
    ...
    lastModified: lastModified(`/services/${service.slug}`),
    changeFrequency: "monthly" as const,
    priority: 0.7,
  }));
  ...
  const projectPages: MetadataRoute.Sitemap = projects.map((project) => ({
    ...
    lastModified: lastModified(`/portfolio/${project.slug}`),
    changeFrequency: "monthly" as const,
    priority: 0.6,

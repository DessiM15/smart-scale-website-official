...
import { isPdf, isReceiptStoreConfigured } from "@/lib/books/receipts";
import { discardReceiptAction } from "../../actions";
import { BOOKS, ReceiptThumb, SnapCard } from "../../../../_components/books";
  ...
  return (
    <Shell active="receipts" banner={params}>
      ...
      <div className="grid lg:grid-cols-[1fr_1.25fr] gap-5 items-start">
        ...
        <Card title={`Waiting · ${pending.length}`} padding="px-5 sm:px-6 pt-5 pb-2" lede="Receipts added but not yet checked and saved. Each one is also on Today.">
          {pending.length === 0 ? (
            <div className="pb-4">
                ...
                <li key={r.id} className="flex items-center gap-4 py-4 border-b border-white/[0.06] last:border-b-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/45 leading-snug">
                      {isPdf(r) ? "Uploaded" : "Snapped"} {formatDate(r.capturedAt.slice(0, 10))}
                      {r.who ? ` by ${r.who}` : ""}
